...
namespace TwoSides.Physics.Entity.NPC
{
    public class BaseCivNpc : BaseNpc
    {
            ...
            Texture2D body, Texture2D legs, Texture2D eye,Texture2D hand, Texture2D shadow)
        {
            ...
            render.Draw(body, Rect, src, effect, Race.GetColor());
            render.Draw(legs,Rect, src,effect, Race.GetColor());
        ...
        public void RenderLeft(Texture2D hand, Render render, float scale = 1.0f)
        {
            ...
            if (Direction < 0)
                effect = SpriteEffects.FlipHorizontally;
            render.Draw(hand, dest, src, effect,Race.GetColor());
        ...
        {
            ClothesSkin[0].Render(render, rect, src, ColorScheme.NotActiveRecipe, 0, effect);
